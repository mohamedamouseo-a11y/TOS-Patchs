...
fail(){ echo "PHASE02_PROJECTS_OBSIDIAN_ELITE_V8=FAIL"; echo "ERROR=$1" >&2; exit "${2:-1}"; }
...
[ "$PATHS" = "$EXPECTED_PATHS" ] || { printf '%s\n' "$STATUS"; fail "Expected exact reviewed V7 working-tree paths only" 17; }
...
def ensure(old,new,token):
    global text
    if token in text: return
    if text.count(old) != 1: raise SystemExit(f"expected exact source target once for {token}, got {text.count(old)}")
...
old_sel='selected && "tos-project-list-row--selected bg-gradient-to-r from-amber-50/90 via-white to-white ring-1 ring-inset ring-amber-200/80 shadow-[0_10px_30px_rgba(245,158,11,0.08)] dark:from-amber-500/10 dark:via-zinc-950 dark:to-zinc-950 dark:ring-amber-500/20"'
new_sel='selected && "tos-project-list-row--selected"'
if old_sel in text:
    if text.count(old_sel) != 1: raise SystemExit('selected utility state duplicated')
    text=text.replace(old_sel,new_sel,1)
elif new_sel not in text:
    raise SystemExit('selected row semantic-only state not found')
...
do grep -Fq "$marker" "$ROOT/$PROJECTS_TARGET" || fail "V8 hook missing: $marker" 20; done
...
rm -rf "$STAGE"; mkdir -p "$STAGE"; cp -a "$DIST/." "$STAGE/"
...
mv "$LIVE" "$BACKUP"
if ! mv "$STAGE" "$LIVE"; then mv "$BACKUP" "$LIVE" || true; fail "Failed to activate V8 live build; rollback attempted" 28; fi
if ! grep -RFlq -- "$V8_RUNTIME" "$LIVE/assets"; then rm -rf "$LIVE"; mv "$BACKUP" "$LIVE" || true; fail "Live V8 runtime sentinel missing; rolled back" 29; fi
if ! grep -RFlq 'tos-projects-archive-toggle' "$LIVE/assets"; then rm -rf "$LIVE"; mv "$BACKUP" "$LIVE" || true; fail "Live Archive hook missing; rolled back" 30; fi
