...
VERSION_ANCHOR = 'export const TCS_MINIMIZED_HEIGHT = 54;'
VERSION_CONST = 'export const TCS_RAMZY_COLLISION_FINAL_VERSION = "v2.4-r1";'
...
NEW_BLOCKED_FUNCTION = '''export function getBlockedRamzyRect(frame, extraGap = 18) {
  ...
  // Open Ramzy: protect the full assistant panel, including its composer.
  // Closed/minimized Ramzy: fall back to protecting the draggable launcher.
...
print("RUNNING=TOS_UXUI_RAMZY_FLAGSHIP_V2_4_R1_TCS_COLLISION_GUARD_FIX")
...
def fail(message: str, launcher_original=None, geometry_original=None):
    ...
    print("RAMZY_FLAGSHIP_V2_4_R1_RUNTIME=NO")
    sys.exit(1)
...
# R1 guard fix: Ramzy is out of scope, so do not require a whole-file Ramzy SHA.
# Instead, prove the approved V2.3 visual layer is present and then preserve Ramzy byte-for-byte.
...
for label, source, anchor in (
    ...
    ("geometry version anchor", geometry_original, VERSION_ANCHOR),
...
if NEW_SELECTOR in launcher_original or VERSION_CONST in geometry_original:
    fail("V2.4 R1 collision fix already applied", launcher_original, geometry_original)
...
next_geometry = geometry_original.replace(VERSION_ANCHOR, VERSION_ANCHOR + "\n" + VERSION_CONST, 1)
...
for token in (
    'TCS_RAMZY_COLLISION_FINAL_VERSION = "v2.4-r1"',
...
# Strict out-of-scope preservation.
...
# Safe live deploy; no service restart and no Git operation in /var/www/TOS.
ts = int(time.time())
candidate = LIVE_PARENT / f"build.ramzy-v2-4-r1-tcs-collision-candidate-{ts}"
backup = LIVE_PARENT / f"build.ramzy-v2-4-r1-tcs-collision-backup-{ts}"
failed_live = LIVE_PARENT / f"build.ramzy-v2-4-r1-tcs-collision-failed-{ts}"
try:
    if candidate.exists():
...
print("RAMZY_FLAGSHIP_V2_4_R1_RUNTIME=YES")
print("R1_RAMZY_WHOLE_FILE_SHA_GUARD=REMOVED_AS_OUT_OF_SCOPE")
print("R1_RAMZY_V2_3_IMPORT_GUARD=PASS")
print("R1_RAMZY_V2_3_CSS_GUARD=PASS")
