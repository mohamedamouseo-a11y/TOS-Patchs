...
PAYLOAD = PATCH_DIR / "TOS_TASK_DETAILS_V2_12_PHASE1_R11.css"
...
if R11_MARKER not in payload_css:
    fail("R11 payload runtime marker missing")
...
for contract in (
    ...
    if contract not in payload_css:
        fail(f"R11 CSS contract missing: {contract}")

app_hash = sha256(APP)
sidebar_hash = sha256(SIDEBAR)
prior_styles = sorted(p for p in STYLE_DIR.glob("taskDetailsV2_12_Phase1*.css") if p != R11_STYLE)
...
stamp = int(time.time())
backup_root = Path(f"/var/backups/tos-patches/task-details-v2-12-phase1-r11-{stamp}")
backup_root.mkdir(parents=True, exist_ok=False)
board_backup = backup_root / BOARD.name
shutil.copy2(BOARD, board_backup)

LIVE.parent.mkdir(parents=True, exist_ok=True)
staging = LIVE.parent / f"build.task-details-v2-12-phase1-r11-staging-{stamp}"
live_backup = LIVE.parent / f"build.task-details-v2-12-phase1-r11-backup-{stamp}"
live_swapped = False
r11_written = False

try:
    BOARD.write_text(updated_board)
    R11_STYLE.write_text(payload_css.rstrip() + "\n")
    r11_written = True
    ...
    if r11_written and R11_STYLE.exists():
        R11_STYLE.unlink()
    if live_swapped:
        if LIVE.exists():
            shutil.rmtree(LIVE)
...
print("TASK_HERO_WIDTH=FULL")
print("RIGHT_EMPTY_LANE_USED=YES")
