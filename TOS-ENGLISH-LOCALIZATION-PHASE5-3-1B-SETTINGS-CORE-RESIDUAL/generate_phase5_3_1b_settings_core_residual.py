...
def main():
    if len(sys.argv) != 3:
        die("usage: generate_phase5_3_1b_settings_core_residual.py REPO_ROOT OUTPUT_PATCH", 2)
    ...
    tmp = Path(tempfile.mkdtemp(prefix="tos-phase5-3-1b-"))
    try:
        run(["git", "init", "-q"], tmp)
        run(["git", "config", "user.email", "phase5-3-1b@tos.local"], tmp)
        run(["git", "config", "user.name", "TOS Phase 5.3.1b Generator"], tmp)
        ...
        if diff.returncode or not diff.stdout.strip():
            die("failed to generate patch", 40)
        ...
        if paths != {TARGET}:
            die(f"unexpected patch paths: {sorted(paths)}", 41)
        print("PARSER=PASS")
        run(["git", "apply", "--check", str(output)], root)
        print("APPLY_CHECK=PASS")
        print("PHASE5_3_1B_SETTINGS_CORE_RESIDUAL_GENERATOR=PASS")
