...
TOP_SECTION_OLD = '              <section className="rounded-[32px] border border-slate-200 bg-white p-5 shadow-sm shadow-slate-100/80 dark:border-white/10 dark:bg-zinc-950 dark:shadow-black/20">'
TOP_SECTION_NEW = '              <section className="rounded-[24px] border border-slate-200 bg-white p-4 shadow-sm shadow-slate-100/70 dark:border-white/10 dark:bg-zinc-950 dark:shadow-black/20">'
...
def main():
    if len(sys.argv) != 3:
        print("Usage: generate_ux_ui_phase08_task_details_v2.py <repo> <output.patch>", file=sys.stderr)
        return 2
    ...
    original_top_section_count = original.count(TOP_SECTION_OLD)
    if original_top_section_count != 4:
        raise RuntimeError(f"TOP_SECTION_DUPLICATE_COUNT={original_top_section_count}; expected 4")
    ...
    if updated.count(TOP_SECTION_OLD) != 3:
        raise RuntimeError(f"NON_TARGET_TASK_DETAIL_SECTIONS_CHANGED={updated.count(TOP_SECTION_OLD)}; expected 3 preserved")
    if updated.count(TOP_SECTION_NEW) != 1:
        raise RuntimeError(f"TARGET_TASK_DETAIL_SECTION_COUNT={updated.count(TOP_SECTION_NEW)}; expected 1")
    ...
    print("ANCHOR11_CONTEXTUAL=YES")
    print("ORIGINAL_DUPLICATE_SECTION_COUNT=4")
    print("NON_TARGET_DUPLICATE_SECTIONS_PRESERVED=3")
