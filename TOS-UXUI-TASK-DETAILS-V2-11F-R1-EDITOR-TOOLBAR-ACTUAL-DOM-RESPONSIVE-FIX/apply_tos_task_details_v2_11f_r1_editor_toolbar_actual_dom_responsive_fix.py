...
PATCH_NAME = "TOS-UXUI-TASK-DETAILS-V2-11F-R1-EDITOR-TOOLBAR-ACTUAL-DOM-RESPONSIVE-FIX"
...
PAYLOAD = PATCH_DIR / "taskDetailsV2_11F_R1EditorToolbarActualDomResponsiveFix.css"

RUNTIME = "--tos-task-details-v2-11f-r1-editor-toolbar-actual-dom-responsive-fix-runtime"
...
if RUNTIME in style_source:
    fail("V2.11F_R1 already applied")
...
# Actual editor DOM from BASE_TOS_COMMIT cd019d...
for marker in (
...
# Explicitly reject the incorrect assumptions from V2.11F.
for fictional in (
    ...
    if fictional in payload_css:
        fail(f"R1 payload contains obsolete/non-existent selector: {fictional}")
...
if RUNTIME not in payload_css:
    fail("R1 runtime marker missing from payload")
for contract in (
    ".tos-task-editor-toolbar",
    ...
    if contract not in payload_css:
        fail(f"R1 toolbar contract missing: {contract}")
...
for forbidden in (
    ...
    if forbidden in payload_css:
        fail(f"R1 payload illegally touches frozen scope: {forbidden}")
...
stamp = int(time.time())
backup_root = Path(f"/var/backups/tos-patches/task-details-v2-11f-r1-{stamp}")
backup_root.mkdir(parents=True, exist_ok=False)
style_backup = backup_root / STYLE.name
shutil.copy2(STYLE, style_backup)
LIVE.parent.mkdir(parents=True, exist_ok=True)
staging = LIVE.parent / f"build.task-details-v2-11f-r1-staging-{stamp}"
live_backup = LIVE.parent / f"build.task-details-v2-11f-r1-backup-{stamp}"
live_swapped = False
...
try:
    ...
    if RUNTIME not in live_css:
        fail("R1 runtime marker missing from live build")
