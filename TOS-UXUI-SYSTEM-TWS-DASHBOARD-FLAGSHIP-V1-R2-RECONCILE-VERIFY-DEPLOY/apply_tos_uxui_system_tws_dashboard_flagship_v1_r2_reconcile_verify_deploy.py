...
print("RUNNING=TOS_UXUI_SYSTEM_TWS_DASHBOARD_FLAGSHIP_V1_R2_RECONCILE_VERIFY_DEPLOY")
...
def fail(message: str):
    ...
    print("TWS_DASHBOARD_FLAGSHIP_V1_R2_RUNTIME=NO")
    sys.exit(1)
...
if actual_page_blob != EXPECTED_PAGE_GIT_BLOB_SHA:
    fail(f"TwsDashboard.jsx current flagship baseline mismatch: {actual_page_blob}")
if actual_style_blob != EXPECTED_STYLE_GIT_BLOB_SHA:
    fail(f"twsDashboardFlagshipV1.css current flagship baseline mismatch: {actual_style_blob}")
...
for marker in required_style_markers:
    if marker not in style_source:
        fail(f"required TWS V1 style marker missing: {marker}")
...
if sha256(PAGE) != page_sha_before:
    fail("TwsDashboard.jsx changed during verify/build")
if sha256(STYLE) != style_sha_before:
    fail("TWS Flagship V1 stylesheet changed during verify/build")
for path in PRESERVE_FILES:
    if sha256(path) != preserve_before[str(path)]:
        fail(f"out-of-scope source changed: {path}")

# Atomic live deploy only; source is never rewritten and no service restart is required.
ts = int(time.time())
candidate = LIVE_PARENT / f"build.tws-dashboard-flagship-v1-r2-candidate-{ts}"
backup = LIVE_PARENT / f"build.tws-dashboard-flagship-v1-r2-backup-{ts}"
failed_live = LIVE_PARENT / f"build.tws-dashboard-flagship-v1-r2-failed-{ts}"
...
# Verify deployed output; rollback live if any marker is absent.
try:
...
print("TWS_DASHBOARD_FLAGSHIP_V1_R2_RUNTIME=YES")
print("TWS_EXISTING_V1_SOURCE=VERIFIED")
print("TWS_SOURCE_REWRITTEN=NO")
