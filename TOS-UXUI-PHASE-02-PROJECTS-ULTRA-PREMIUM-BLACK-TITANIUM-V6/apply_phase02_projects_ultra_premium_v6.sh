...
SOURCE_APPEND="$PATCH_DIR/projects-ultra-premium-black-titanium-v6.append.css"
...
STAGE="$LIVE_PARENT/build.phase02-ultra-v6.new.$$"
BACKUP="$LIVE_PARENT/build.phase02-ultra-v6.backup-$STAMP"

fail() {
  echo "PHASE02_PROJECTS_ULTRA_PREMIUM_V6=FAIL"
  echo "ERROR=$1" >&2
  exit "${2:-1}"
}
...
[ -f "$SOURCE_APPEND" ] || fail "Missing V6 CSS source" 4
...
[ "$(git -C "$PATCH_REPO_ROOT" hash-object "$SOURCE_APPEND")" = "$(git -C "$PATCH_REPO_ROOT" rev-parse "HEAD:$SOURCE_REL")" ] || fail "V6 patch source differs from TOS-Patchs HEAD" 9
...
V6_COUNT="$(grep -Fc -- "$V6_RUNTIME" "$ROOT/$CSS_TARGET" || true)"
[ "$V6_COUNT" -le 1 ] || fail "Duplicate V6 runtime sentinel" 16

if [ "$V6_COUNT" = "0" ]; then
  CURRENT_SHA="$(sha256sum "$ROOT/$CSS_TARGET" | awk '{print $1}')"
  echo "BASELINE_CSS_SHA256=$CURRENT_SHA"
  [ "$CURRENT_SHA" = "$EXPECTED_V4_CSS_SHA256" ] || fail "Projects CSS is not exact reviewed V4 baseline" 17
  printf '\n' >> "$ROOT/$CSS_TARGET"
  cat "$SOURCE_APPEND" >> "$ROOT/$CSS_TARGET"
  PATCH_ACTION="APPLIED_ULTRA_PREMIUM_V6"
else
  PATCH_ACTION="VALIDATED_EXISTING_ULTRA_PREMIUM_V6"
fi

[ "$(grep -Fc -- "$V6_RUNTIME" "$ROOT/$CSS_TARGET" || true)" = "1" ] || fail "V6 runtime sentinel missing in source" 18
...
grep -RFlq -- "$V6_RUNTIME" "$DIST/assets" || fail "V6 runtime sentinel missing from dist assets" 20

rm -rf "$STAGE"
mkdir -p "$STAGE"
cp -a "$DIST/." "$STAGE/"
[ -f "$STAGE/index.html" ] || fail "Staged index missing" 21
grep -RFlq -- "$V6_RUNTIME" "$STAGE/assets" || fail "V6 runtime sentinel missing from staged assets" 22

mv "$LIVE" "$BACKUP"
if ! mv "$STAGE" "$LIVE"; then
  mv "$BACKUP" "$LIVE" || true
  fail "Failed to activate V6 live build; rollback attempted" 23
fi

if ! grep -RFlq -- "$V6_RUNTIME" "$LIVE/assets"; then
  rm -rf "$LIVE"
  mv "$BACKUP" "$LIVE" || true
  fail "Live V6 runtime sentinel missing; rolled back" 24
fi
...
echo "PHASE02_PROJECTS_ULTRA_PREMIUM_V6=PASS"
echo "SCREEN=Projects"
echo "PATCH_ACTION=$PATCH_ACTION"
echo "DESIGN_SYSTEM=BLACK_TITANIUM_CHAMPAGNE"
echo "CONTRAST_AUDIT=APPLIED"
