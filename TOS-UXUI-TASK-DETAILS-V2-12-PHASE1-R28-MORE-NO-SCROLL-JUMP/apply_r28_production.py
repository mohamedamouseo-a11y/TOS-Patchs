...
for contract in (r27_import, r27_ref_line.strip(), r27_effect_start, r27_effect_end, scroll_fn, 'setTaskMoreDetailsOpen(nextOpen);'):
    if contract not in board_source:
        fail(f"R28 source contract missing: {contract[:90]}")
...
# Remove R27 manual scroll compensation; keep R27 harmless CSS clip/safe-margin guard.
...
helper = '''  function scrollTaskDetailsToTop() {
  ...
  function setTaskMoreDetailsOpenStable(nextOpen) {
    ...
    const restoreScrollTop = () => {
      ...
      currentScroller.scrollTop = frozenScrollTop;
...
# Only replace user-triggered More toggles; task-change reset remains setTaskMoreDetailsOpen(false).
toggle_call = 'setTaskMoreDetailsOpen(nextOpen);'
toggle_count = updated.count(toggle_call)
if toggle_count != 3:
    fail(f"expected exactly three More toggle calls, found {toggle_count}")
updated = updated.replace(toggle_call, 'setTaskMoreDetailsOpenStable(nextOpen);')
...
for contract in (
    "TOS_TASK_DETAILS_R28_MORE_NO_SCROLL_JUMP",
    "setTaskMoreDetailsOpenStable(nextOpen);",
    'currentScroller.scrollTop = frozenScrollTop;',
    'currentScroller.dataset.tosR28MoreNoScrollJump = "true"',
    r28_import,
):
    if contract not in updated:
        fail(f"R28 source contract missing after edit: {contract}")
...
stamp = int(time.time())
backup_root = Path(f"/var/backups/tos-patches/task-details-v2-12-phase1-r28-{stamp}")
backup_root.mkdir(parents=True, exist_ok=False)
shutil.copy2(BOARD, backup_root / BOARD.name)
LIVE.parent.mkdir(parents=True, exist_ok=True)
staging = LIVE.parent / f"build.task-details-r28-staging-{stamp}"
live_backup = LIVE.parent / f"build.task-details-r28-backup-{stamp}"
live_swapped = False
r28_written = False
...
try:
    ...
    for token in ("tosR28MoreNoScrollJump", "frozenScrollTop"):
        if token not in built_js:
            fail(f"R28 runtime token missing from build JS: {token}")
    if "r27MoreGeometryRef" in built_js:
        fail("R27 manual compensation survived in built JS")
    ...
    for token in ("tosR28MoreNoScrollJump", "frozenScrollTop"):
        if token not in live_js:
            fail(f"R28 runtime token missing from live JS: {token}")
    if "r27MoreGeometryRef" in live_js:
        fail("R27 manual compensation survived in live JS")
