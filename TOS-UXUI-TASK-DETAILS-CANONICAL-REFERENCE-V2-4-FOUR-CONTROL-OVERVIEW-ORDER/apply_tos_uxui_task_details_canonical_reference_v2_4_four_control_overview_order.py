...
PAYLOAD_STYLE = PATCH_DIR / "taskDetailsCanonicalReferenceV2_4FourControlOverviewOrder.css"

RUNTIME = "--tos-task-details-canonical-reference-v2-4-four-control-overview-order-runtime"
...
CONTROLS_OPEN_PATTERN = re.compile(
    r'(<div\s+className="[^"]*\btos-task-summary-controls\b[^"]*">)'
)
...
if RUNTIME not in payload_css:
    fail("V2.4 CSS runtime marker missing from payload")

# V2.4 must not reopen already-approved App Shell / Topbar / Tabs scope.
for forbidden in (
    ".tos-premium-sidebar",
    ...
    if forbidden in payload_css:
        fail(f"V2.4 payload illegally touches preserved scope: {forbidden}")
...
for required_selector in (
    ...
    if required_selector not in payload_css:
        fail(f"V2.4 required selector missing from payload: {required_selector}")
...
if "tos-task-assignees-hero-card" in board_source:
    fail("V2.4 Assignees Hero control already present")

controls_matches = list(CONTROLS_OPEN_PATTERN.finditer(board_source))
if len(controls_matches) != 1:
    fail(f"expected exactly one Task Details summary controls container, found {len(controls_matches)}")

if board_source.count("tos-task-start-date-card") != 1:
    fail("expected exactly one Start date Hero card before V2.4")

# Locate the single canonical stylesheet with the full V2 -> V2.3 lineage.
v2_candidates = []
for path in STYLE_DIR.rglob("*.css"):
    text = path.read_text(errors="ignore")
    if RUNTIME in text:
        fail(f"V2.4 already applied in {path}")
    if (
        V2_3_RUNTIME in text
...
if len(v2_candidates) != 1:
    fail(
        "unable to identify exactly one V2+V2.1+V2.2+V2.3 stylesheet: "
        + ", ".join(str(path) for path in v2_candidates)
    )
v2_style = v2_candidates[0]
original_css = v2_style.read_text()

for predecessor_marker in (V2_3_RUNTIME, V2_2_RUNTIME, V2_1_RUNTIME):
    if predecessor_marker not in original_css:
        fail(f"required predecessor runtime missing: {predecessor_marker}")
...
stamp = int(time.time())
backup_root = Path(
    f"/var/backups/tos-patches/task-details-canonical-reference-v2-4-four-control-overview-order-{stamp}"
)
...
staging = live.parent / f"build.task-details-canonical-reference-v2-4-staging-{stamp}"
live_backup = live.parent / f"build.task-details-canonical-reference-v2-4-backup-{stamp}"
live_swapped = False

try:
    # Insert Assignees as the first primary Hero control. Start date stays in source
    # and is exposed by CSS only when the existing More state is open.
    updated_board, insert_count = CONTROLS_OPEN_PATTERN.subn(
        lambda match: match.group(1) + ASSIGNEE_CARD,
        board_source,
        count=1,
    )
    if insert_count != 1:
        fail(f"Assignees Hero control insertion failed: {insert_count}")

    if updated_board.count("tos-task-assignees-hero-card") != 1:
        fail("Assignees Hero control marker missing or duplicated after transform")
    if updated_board.count("tos-task-start-date-card") != 1:
        fail("Start date advanced control was not preserved")
    if 'data-content-dir={modalDirection}' not in updated_board:
        fail("V2.2 structural/content direction marker lost during V2.4 transform")

    BOARD.write_text(updated_board)

    v2_style.write_text(original_css.rstrip() + "\n\n" + payload_css.strip() + "\n")
    updated_css = v2_style.read_text()
    for marker in (RUNTIME, V2_3_RUNTIME, V2_2_RUNTIME, V2_1_RUNTIME):
    ...
    for marker in (
        ...
        if marker not in built_js:
            fail(f"required V2.4/V2.2 marker missing from built JS: {marker}")
    ...
    if RUNTIME not in live_css or V2_3_RUNTIME not in live_css:
        fail("V2.4/V2.3 markers missing from live build")
    if "tos-task-assignees-hero-card" not in live_js or "data-content-dir" not in live_js:
        fail("V2.4 Assignees or V2.2 direction marker missing from live JS")

    print("PASS/FAIL=PASS")
    print("BUILD_RESULT=PASS")
    print("LIVE_DEPLOY=PASS")
    print("TASK_DETAILS_CANONICAL_REFERENCE_V2_4_RUNTIME=YES")
    ...
    print("ASSIGNEES_HERO_CONTROL=ADDED_USING_EXISTING_ASSIGNEE_STATE")
    print("ASSIGNEES_ACTION=OPENS_EXISTING_SIDE_DETAILS_PATH")
    print("START_DATE=ADVANCED_MORE_PATH_PRESERVED")
    print("OVERVIEW_PHYSICAL_ORDER=DESCRIPTION_LEFT_RIGHT_RAIL_RIGHT")
    print("RIGHT_RAIL_CONTENT=QUICK_ACTIONS_TASK_INFORMATION_TAGS_TCS_ASSISTANT_PRESERVED")
