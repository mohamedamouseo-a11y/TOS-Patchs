...
VERSION = "TOS_TASK_DETAILS_V2_8D"
PATCH_NAME = "TOS-UXUI-TASK-DETAILS-V2-8D-RIGHT-RAIL-TCS-VIEWPORT-FIT"
BASELINE_CHAIN = "TOS_TASK_DETAILS_V2_7"
...
PAYLOAD = PATCH_DIR / "taskDetailsV2_8DRightRailTcsViewportFit.css"

RUNTIME = "--tos-task-details-v2-8d-right-rail-tcs-viewport-fit-runtime"
...
# Guard the exact V2.7 chain and existing canonical Overview rail/TCS card.
for marker in (
...
if board_source.count("tos-task-reference-v2-rail") != 1:
    fail("expected exactly one canonical Overview rail")
if board_source.count("tos-task-v2-tcs-card") != 1:
    fail("expected exactly one existing Task Details TCS Assistant card")
...
if RUNTIME in style_source:
    fail("V2.8D already applied")

# V2.7 physical geometry must already be present. V2.8D is density-only.
for marker in (
    ".tos-task-reference-v2-rail",
    "right: 0 !important",
    ".tos-task-description-panel",
    ...
    if marker not in style_source:
        fail(f"required V2.7 physical geometry marker missing: {marker}")

if RUNTIME not in payload_css:
    fail("V2.8D runtime marker missing from payload")
for required_selector in (
    ...
    if required_selector not in payload_css:
        fail(f"required V2.8D selector missing from payload: {required_selector}")

# Freeze everything outside canonical rail density.
for forbidden in (
    ".tos-premium-sidebar",
    ...
    if forbidden in payload_css:
        fail(f"V2.8D payload illegally touches frozen scope: {forbidden}")
...
stamp = int(time.time())
backup_root = Path(f"/var/backups/tos-patches/task-details-v2-8d-right-rail-{stamp}")
backup_root.mkdir(parents=True, exist_ok=False)
style_backup = backup_root / STYLE.name
shutil.copy2(STYLE, style_backup)

live.parent.mkdir(parents=True, exist_ok=True)
staging = live.parent / f"build.task-details-v2-8d-staging-{stamp}"
live_backup = live.parent / f"build.task-details-v2-8d-backup-{stamp}"
live_swapped = False
...
try:
    ...
    if sha256(BOARD) != board_hash_before:
        fail("ProfessionalTaskBoard.jsx changed during CSS-only V2.8D patch")
    ...
    if RUNTIME not in live_css or V27_RUNTIME not in live_css:
        fail("V2.8D/V2.7 runtime marker missing from live CSS")
    ...
    print("RIGHT_RAIL_PHYSICAL_POSITION=PRESERVED_RIGHT")
    print("DESCRIPTION_PHYSICAL_POSITION=PRESERVED_LEFT")
    print("RIGHT_RAIL_CONTENT=PRESERVED")
    print("TASK_DETAILS_TCS_CARD=EXISTING_CARD_VIEWPORT_FIT_TARGET")
    print("HERO_AND_FOUR_CONTROLS=PRESERVED")
