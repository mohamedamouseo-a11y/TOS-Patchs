...
VERSION = "TOS_TASK_DETAILS_V2_11B"
PATCH_NAME = "TOS-UXUI-TASK-DETAILS-V2-11B-FOUR-HERO-CONTROLS-PREMIUM-LOCK"
BASELINE_CHAIN = "TOS_TASK_DETAILS_V2_11A_R2"
MICRO_STEP = "FOUR_HERO_CONTROLS_ONLY"
...
PAYLOAD = PATCH_DIR / "taskDetailsV2_11BFourHeroControlsPremiumLock.css"

RUNTIME = "--tos-task-details-v2-11b-four-hero-controls-premium-lock-runtime"
R2_RUNTIME = "--tos-task-details-v2-11a-r2-hero-width-only-runtime"
REQUIRED_LINEAGE = (
...
# Exact live DOM markers needed for this micro-step.
for marker in (
    "tos-task-details-reference-v2",
    "tos-task-summary-compact",
    "tos-task-summary-controls",
    "tos-task-v2-assignees-card",
    "tos-task-v2-control-label",
    "tos-task-v2-assignee-content",
    "tos-task-v2-assignee-avatar",
    "tos-task-v2-assignee-add",
    "tos-task-date-input",
    "tos-task-start-date-card",
):
    if marker not in board_source:
        fail(f"required Task Details control marker missing: {marker}")

# Preserve the canonical exact-four-control DOM: no stale duplicate injected card.
...
if RUNTIME in style_source:
    fail("V2.11B already applied")

# Guard the specific pre-V2.11B control lineage being superseded visually.
for marker in (
    "transform: translateY(-20px)",
    "--tos-v29d-control-box-height: 76px",
    R2_RUNTIME,
):
    if marker not in style_source:
        fail(f"required V2.11A_R2 baseline marker missing: {marker}")

if RUNTIME not in payload_css:
    fail("V2.11B runtime marker missing from payload")
...
for required_selector in (
    ...
    if required_selector not in payload_css:
        fail(f"required V2.11B selector missing from payload: {required_selector}")

# MICRO STEP boundary: controls only. Hero shell/identity and everything below tabs are frozen.
for forbidden in (
    ".tos-task-summary-compact {",
    ...
    if forbidden in payload_css:
        fail(f"V2.11B payload illegally touches frozen micro-step scope: {forbidden}")
...
stamp = int(time.time())
backup_root = Path(f"/var/backups/tos-patches/task-details-v2-11b-controls-{stamp}")
backup_root.mkdir(parents=True, exist_ok=False)
style_backup = backup_root / STYLE.name
shutil.copy2(STYLE, style_backup)

live.parent.mkdir(parents=True, exist_ok=True)
staging = live.parent / f"build.task-details-v2-11b-staging-{stamp}"
live_backup = live.parent / f"build.task-details-v2-11b-backup-{stamp}"
live_swapped = False
...
try:
    ...
    if sha256(BOARD) != board_hash_before:
        fail("ProfessionalTaskBoard.jsx changed during CSS-only V2.11B patch")
    ...
    for marker in (
        ...
        if marker not in built_js:
            fail(f"required control marker missing from built JS: {marker}")
    ...
    if RUNTIME not in live_css or R2_RUNTIME not in live_css:
        fail("V2.11B/V2.11A_R2 runtime marker missing from live CSS")
    for marker in ("tos-task-summary-controls", "tos-task-v2-assignees-card", "tos-task-date-input"):
    ...
    print("CONTROL_COUNT=4")
    print("CONTROL_ORDER=ASSIGNEES_STATUS_PRIORITY_DUE_DATE")
    print("CONTROL_VISUALS=PREMIUM_EQUAL_CARDS_TARGET")
    print("HERO_SHELL_CHANGED=NO")
    print("TASK_IDENTITY_CHANGED=NO")
