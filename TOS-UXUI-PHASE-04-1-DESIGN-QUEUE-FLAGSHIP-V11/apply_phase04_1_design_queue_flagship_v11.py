...
print("RUNNING=PHASE04_1_DESIGN_QUEUE_FLAGSHIP_V11")
...
/* =========================================================
   ...
   Screenshot review after V10 showed the portal/options correctly themed,
   while Chrome still painted the focused searchable project input white in
   dark mode. Force the portal search control itself with higher specificity
   so global/native input rules cannot override the dark surface.
   ========================================================= */
...
/* Keep the light search deliberately porcelain, not browser-default gray. */
...
updated_css = original_css.rstrip() + v11_css + "\n"
backup = None
stage = None
live_swapped = False
...
try:
    ...
    stage = LIVE_PARENT / f"build.phase04-1-design-queue-v11.new.{int(time.time())}"
    backup = LIVE_PARENT / f"build.phase04-1-design-queue-v11.backup-{stamp}"
    if stage.exists():
        shutil.rmtree(stage)
