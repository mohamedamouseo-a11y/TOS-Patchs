...
def main():
    ...
    start_count = before.count(DETAILS_START)
    global_end_count = before.count(DETAILS_END)
    if start_count != 1:
        raise RuntimeError(f'DETAILS_START_COUNT={start_count}; expected 1')
    if global_end_count != 2:
        raise RuntimeError(f'GLOBAL_DETAILS_END_COUNT={global_end_count}; expected 2')

    start = before.index(DETAILS_START)
    tail = before[start:]
    post_start_end_count = tail.count(DETAILS_END)
    if post_start_end_count != 1:
        raise RuntimeError(f'POST_START_DETAILS_END_COUNT={post_start_end_count}; expected 1')

    end_marker_rel = tail.index(DETAILS_END)
    end = start + end_marker_rel + len(DETAILS_CLOSE)
    prefix = before[:start]
    ...
    if after[:len(prefix)] != prefix:
        raise RuntimeError('UI12_PREFIX_CHANGED')
    if after[len(prefix) + len(updated):] != suffix:
        raise RuntimeError('UI12_SUFFIX_CHANGED')
    ...
    print('PREVIOUS_FAILURE=DETAILS_END_COUNT_2')
    print('DETAILS_END_CONTEXTUAL=YES')
    print(f'GLOBAL_DETAILS_END_COUNT={global_end_count}')
    print(f'POST_START_DETAILS_END_COUNT={post_start_end_count}')
