...
PATCH = "TOS-TWS-TDOCS-PHASE-1-GOOGLE-DOCS-UX-DARK-FIDELITY"
SCRIPT = "run_tdocs_phase1.py"
...
CSS = "frontend/src/pages/tws/tDocsPhase1GoogleDocsFidelity.css"
TEST = "frontend/src/pages/tws/tDocsPhase1GoogleDocsFidelity.test.js"
PHASE_SCOPE = {EDITOR, CSS, TEST}

BASE_EDITOR_BLOB = "42e1c14bb2f0eb6dc6ce0b3b90f925ffd4087e9a"
EXPECTED_CSS_BLOB = "3fe80f54c62a8fc8076a4e77da1e2f2d35c95b9f"
EXPECTED_TEST_BLOB = "2d359674c338a7c6ca810d12508b08173feaabee"

CSS_URL = "https://raw.githubusercontent.com/mohamedamouseo-a11y/TOS-Patchs/main/TOS-TWS-TDOCS-PHASE-1-GOOGLE-DOCS-UX-DARK-FIDELITY/payload/tDocsPhase1GoogleDocsFidelity.css"
TEST_URL = "https://raw.githubusercontent.com/mohamedamouseo-a11y/TOS-Patchs/main/TOS-TWS-TDOCS-PHASE-1-GOOGLE-DOCS-UX-DARK-FIDELITY/payload/tDocsPhase1GoogleDocsFidelity.test.js"
...
def fetch_bytes(url: str) -> bytes:
    req = urllib.request.Request(
        url,
        headers={"User-Agent": "TOS-TDocs-Phase1/1.0", "Cache-Control": "no-cache", "Pragma": "no-cache"},
    )
...
try:
    ...
    if head_editor_blob != BASE_EDITOR_BLOB:
        raise RuntimeError(
            f"TDocsEditor baseline changed in current HEAD: expected {BASE_EDITOR_BLOB}, got {head_editor_blob}"
        )

    pre_paths = changed_paths()
    overlap = pre_paths & PHASE_SCOPE
    if overlap:
        raise RuntimeError("dirty overlap with T-Docs Phase 1 scope: " + ", ".join(sorted(overlap)))
    ...
    if git("hash-object", EDITOR) != BASE_EDITOR_BLOB:
        raise RuntimeError("TDocsEditor working-tree blob does not match guarded baseline")
    pre_editor_bytes = editor_path.read_bytes()
    source = pre_editor_bytes.decode("utf-8")

    if "tDocsPhase1GoogleDocsFidelity.css" in source or "tws-docs-phase1-google-fidelity" in source:
        raise RuntimeError("T-Docs Phase 1 already appears applied")
    for rel in (CSS, TEST):
        if (REPO / rel).exists():
    ...
    if "tests 10" not in (phase_test.stdout or "") and "pass 10" not in (phase_test.stdout or ""):
        pass
    ...
    live_before_html = fetch_text(f"{LIVE_URL}?tdocs_phase1_before={int(time.time())}")
    ...
    backup_dir = Path("/tmp") / f"tdocs_phase1_live_backup_{int(time.time())}"
    ...
    live_after_html = fetch_text(f"{LIVE_URL}?tdocs_phase1_after={int(time.time())}")
    ...
    live_asset_bytes = fetch_bytes(f"{asset_url(live_asset_after)}?tdocs_phase1={int(time.time())}")
    ...
    print("TDOCS_BASELINE_BLOB_GUARD=PASS")
    print("PAYLOAD_INTEGRITY=PASS")
    print("FILES_PATCHED=3")
    print("TDOCS_PHASE1_TESTS=PASS (10/10)")
    print("TDOCS_DARK_PAPER_READABILITY=PASS")
    print("TDOCS_DARK_TITLE_VISIBILITY=PASS")
    print("TDOCS_COMPACT_TOOLBAR=PASS")
    print("TDOCS_RULER=PASS")
    print("TDOCS_OUTLINE_INFO_DARK_CONTRAST=PASS")
    print("TDOCS_PAGE_CANVAS_FIDELITY=PASS")
