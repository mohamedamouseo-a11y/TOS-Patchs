...
EXPECTED_ORIGINAL_BLOB = "a82bb7aecbba3a32d657118af3f5881fcebb2dfb"
...
def main():
    if len(sys.argv) != 4:
        die("usage: generate_batch4_3_1_team_v3.py ORIGINAL_GENERATOR REPO_ROOT OUTPUT_PATCH", 2)
    ...
    print(f"V3_REPAIR_MATCHES={count}")
    if count != 1:
        die(f"expected exactly one replacement-11 expectation line, found {count}", 6)

    corrected = source.replace(OLD, NEW)
    tmp = Path(tempfile.mkdtemp(prefix="tos-batch4-3-1-v3-")) / "generate_batch4_3_1_team_corrected.py"
    ...
    print("GENERATOR_V3_REPAIR=PASS")
