...
PATCH_NAME = "TOS-UXUI-TASK-DETAILS-V2-11A-R2-HERO-WIDTH-ONLY"
...
PAYLOAD = PATCH_DIR / "taskDetailsV2_11A_R2HeroWidthOnly.css"

RUNTIME = "--tos-task-details-v2-11a-r2-hero-width-only-runtime"
R1_RUNTIME = "--tos-task-details-v2-11a-r1-hero-full-canvas-physical-placement-runtime"
REQUIRED_LINEAGE = (
...
if RUNTIME in style_source:
    fail("V2.11A_R2 already applied")

# Guard the exact specificity conflict being repaired.
for marker in (
    "body:has(.tos-task-details-reference-v1[data-content-dir]) .tos-task-details-layout",
    "grid-template-columns: minmax(0, 1fr) var(--tos-v25-right-rail-width)",
    "--tos-task-details-v2-11a-r1-hero-full-canvas-physical-placement-runtime",
):
    if marker not in style_source:
        fail(f"required R1/V2.5 baseline marker missing: {marker}")

if RUNTIME not in payload_css:
    fail("V2.11A_R2 runtime marker missing from payload")
...
for required_selector in (
    ...
    if required_selector not in payload_css:
        fail(f"required R2 selector missing from payload: {required_selector}")

# Width-only micro-step boundary: all inner Hero content and lower-page UI stay frozen.
for forbidden in (
    ".tos-task-reference-title-block",
...
stamp = int(time.time())
backup_root = Path(f"/var/backups/tos-patches/task-details-v2-11a-r2-hero-width-{stamp}")
backup_root.mkdir(parents=True, exist_ok=False)
style_backup = backup_root / STYLE.name
shutil.copy2(STYLE, style_backup)

live.parent.mkdir(parents=True, exist_ok=True)
staging = live.parent / f"build.task-details-v2-11a-r2-staging-{stamp}"
live_backup = live.parent / f"build.task-details-v2-11a-r2-backup-{stamp}"
live_swapped = False
...
try:
    ...
    if sha256(BOARD) != board_hash_before:
        fail("ProfessionalTaskBoard.jsx changed during CSS-only V2.11A_R2 patch")
    ...
    if RUNTIME not in live_css or R1_RUNTIME not in live_css:
        fail("R2/R1 runtime marker missing from live CSS")
    for marker in ("tos-task-summary-compact", "tos-task-reference-v2-rail"):
    ...
    print("ROOT_CAUSE=V2_5_HIGH_SPECIFICITY_GRID_RULE_OUTRANKED_R1")
    print("LAYOUT_CONTEXT=FORCED_FULL_WIDTH_BLOCK")
    print("MAIN_COLUMN=FORCED_100_PERCENT")
    print("HERO_WIDTH=FORCED_100_PERCENT")
    print("INNER_HERO_CONTENT_CHANGED=NO")
    print("CONTROLS_CHANGED=NO")
