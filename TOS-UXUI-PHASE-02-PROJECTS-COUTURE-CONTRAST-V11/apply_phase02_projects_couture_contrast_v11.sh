...
SOURCE_APPEND="$PATCH_DIR/projects-couture-contrast-v11.append.css"
...
EXPECTED_V10_CSS_SHA256="c281bc05a9b729bdfdad6169f145b8bc198666bbedb69f96d20c87927bbe4f47"
...
STAGE="$LIVE_PARENT/build.phase02-couture-v11.new.$$"
BACKUP="$LIVE_PARENT/build.phase02-couture-v11.backup-$STAMP"

fail() {
  echo "PHASE02_PROJECTS_COUTURE_CONTRAST_V11=FAIL"
  echo "ERROR=$1" >&2
  exit "${2:-1}"
}
...
[ -f "$SOURCE_APPEND" ] || fail "Missing V11 CSS source" 4
...
[ "$(git -C "$PATCH_REPO_ROOT" hash-object "$SOURCE_APPEND")" = "$(git -C "$PATCH_REPO_ROOT" rev-parse "HEAD:$SOURCE_REL")" ] || fail "V11 patch source differs from TOS-Patchs HEAD" 9
...
[ "$(grep -Fc -- "$V10_RUNTIME" "$ROOT/$CSS_TARGET" || true)" = "1" ] || fail "V10 runtime baseline missing" 14
...
[ "$PATHS" = "$EXPECTED_PATHS" ] || {
  ...
  printf '%s\n' "$STATUS"
  fail "Expected exact reviewed V10 working-tree paths only" 19
}

V11_COUNT="$(grep -Fc -- "$V11_RUNTIME" "$ROOT/$CSS_TARGET" || true)"
[ "$V11_COUNT" -le 1 ] || fail "Duplicate V11 runtime sentinel" 20

if [ "$V11_COUNT" = "0" ]; then
  CURRENT_SHA="$(sha256sum "$ROOT/$CSS_TARGET" | awk '{print $1}')"
  echo "BASELINE_CSS_SHA256=$CURRENT_SHA"
  [ "$CURRENT_SHA" = "$EXPECTED_V10_CSS_SHA256" ] || fail "Projects CSS is not exact reviewed V10 baseline" 21
  printf '\n' >> "$ROOT/$CSS_TARGET"
  cat "$SOURCE_APPEND" >> "$ROOT/$CSS_TARGET"
  PATCH_ACTION="APPLIED_COUTURE_CONTRAST_V11"
else
  PATCH_ACTION="VALIDATED_EXISTING_COUTURE_V11"
fi

[ "$(grep -Fc -- "$V11_RUNTIME" "$ROOT/$CSS_TARGET" || true)" = "1" ] || fail "V11 runtime sentinel missing in source" 22
...
grep -RFlq -- "$V11_RUNTIME" "$DIST/assets" || fail "V11 runtime sentinel missing from dist assets" 24
grep -RFlq 'tos-projects-archive-toggle' "$DIST/assets" || fail "Archive hook missing from dist assets" 25

grep -RFlq 'tos-project-inspector' "$DIST/assets" || fail "Inspector hook missing from dist assets" 26

rm -rf "$STAGE"
mkdir -p "$STAGE"
cp -a "$DIST/." "$STAGE/"
[ -f "$STAGE/index.html" ] || fail "Staged index missing" 27
grep -RFlq -- "$V11_RUNTIME" "$STAGE/assets" || fail "V11 runtime sentinel missing from staged assets" 28

mv "$LIVE" "$BACKUP"
if ! mv "$STAGE" "$LIVE"; then
  mv "$BACKUP" "$LIVE" || true
  fail "Failed to activate V11 live build; rollback attempted" 29
fi

if ! grep -RFlq -- "$V11_RUNTIME" "$LIVE/assets"; then
  rm -rf "$LIVE"
  mv "$BACKUP" "$LIVE" || true
  fail "Live V11 runtime sentinel missing; rolled back" 30
fi
...
echo "PHASE02_PROJECTS_COUTURE_CONTRAST_V11=PASS"
echo "SCREEN=Projects"
echo "PATCH_ACTION=$PATCH_ACTION"
echo "DESIGN_SYSTEM=COUTURE_IVORY_OBSIDIAN_CHAMPAGNE"
echo "LIGHT_INSPECTOR_CONTRAST=FIXED"
echo "ARCHIVE_CONTROL=COUTURE_VAULT"
