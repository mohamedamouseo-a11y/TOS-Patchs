...
def ensure_replace(text, old, new, label):
    if new in text:
        return text
    if old not in text:
        raise SystemExit(f'PHASE10_V2_RESILIENT_ERROR={label}_STATE_UNKNOWN')
...
export function scoreIdentityNameMatch(queryRaw, valueRaw) {
  ...
  // A single spoken/written first name may match a longer canonical name.
  // Duplicate candidates are still forced through the shared ambiguity guard.
  ...
  const score = Math.min(
    0.89,
    0.69 + coverage.queryCoverage * 0.09 + coverage.valueCoverage * 0.04 + coverage.meanScore * 0.08,
  );
  return {
    score,
...
if (ROOT / identity_path).exists():
    ...
    if 'RAMZY_IDENTITY_NAME_MATCHING_V1' not in existing:
        raise SystemExit('PHASE10_V2_RESILIENT_ERROR=IDENTITY_SERVICE_CONFLICT')
else:
    write(identity_path, identity_service)

# entityResolution.service.js — fresh or partially-applied Phase 10 are both valid.
...
text = ensure_replace(text, 'version: "RAMZY_ENTITY_RESOLUTION_V1",', 'version: "RAMZY_ENTITY_RESOLUTION_V2",', 'ENTITY_RESOLUTION_VERSION')
if 'identityNameMatchingVersion: "RAMZY_IDENTITY_NAME_MATCHING_V1"' not in text:
    anchor = '    fuzzyMultilingualVersion: "RAMZY_FUZZY_MULTILINGUAL_V1",\n'
    if anchor not in text:
        raise SystemExit('PHASE10_V2_RESILIENT_ERROR=ENTITY_RESOLUTION_CONFIG_STATE_UNKNOWN')
    text = text.replace(anchor, anchor + '    identityNameMatchingVersion: "RAMZY_IDENTITY_NAME_MATCHING_V1",\n', 1)
write(rel, text)

# entityAlias.service.js — fresh or partially-applied Phase 10 are both valid.
rel = 'backend/src/agency-operator/services/entityAlias.service.js'
text = read(rel)
if 'import { scoreIdentityNameMatch } from "./identityNameMatching.service.js";' not in text:
    anchor = 'import { normalizeEntityText } from "./entityResolution.service.js";\n'
    if anchor not in text:
        raise SystemExit('PHASE10_V2_RESILIENT_ERROR=ENTITY_ALIAS_IMPORT_STATE_UNKNOWN')
    text = text.replace(anchor, anchor + 'import { scoreIdentityNameMatch } from "./identityNameMatching.service.js";\n', 1)
...
if 'resolutionMatchType: "ALIAS_EXACT"' not in text:
    if old_fn not in text:
        raise SystemExit('PHASE10_V2_RESILIENT_ERROR=ENTITY_ALIAS_FIND_STATE_UNKNOWN')
    text = text.replace(old_fn, new_fn, 1)
text = ensure_replace(text, 'version: "RAMZY_ALIAS_LEARNING_V1",', 'version: "RAMZY_ALIAS_LEARNING_V2",', 'ENTITY_ALIAS_VERSION')
if 'spellingAwareLookup: true' not in text:
    anchor = '    learningMode: "EXPLICIT_USER_ONLY",\n'
    if anchor not in text:
        raise SystemExit('PHASE10_V2_RESILIENT_ERROR=ENTITY_ALIAS_CONFIG_STATE_UNKNOWN')
    text = text.replace(anchor, anchor + '    spellingAwareLookup: true,\n    fuzzyAliasAutoResolve: false,\n', 1)
write(rel, text)

# ramzySystemIntelligence.service.js — replace however many old blocks remain.
...
if 'aliasField: "aliasMatch"' in text or 'aliasUsed: Boolean(resolution.entity?.aliasMatch)' in text:
    raise SystemExit('PHASE10_V2_RESILIENT_ERROR=SYSTEM_INTELLIGENCE_STALE_ALIAS_STATE')
if text.count('aliasField: "aliasConfidence"') < 2 or text.count('aliasConfidence: aliases') < 2:
    raise SystemExit('PHASE10_V2_RESILIENT_ERROR=SYSTEM_INTELLIGENCE_ALIAS_BRIDGE_INCOMPLETE')
write(rel, text)

# Prompt rule is idempotent.
...
if '- Phase 10:' not in text:
    ...
    if anchor not in text:
        raise SystemExit('PHASE10_V2_RESILIENT_ERROR=RAMZY_PROMPT_ANCHOR_MISSING')
...
# Focused Phase 10 identity tests; safe to leave an identical/known prior file.
...
test("Phase 10 auto-resolves one authorized phonetic candidate but clarifies duplicates", () => {
  const one = resolveEntityCandidates({ query: "يوسف", candidates: [{ id: "u1", name: "Youssef Ahmed" }], fields: ["name"], exactFields: ["name"], entityType: "USER" });
  assert.equal(one.guardDecision, "AUTO_RESOLVE");
  assert.equal(one.entity?.id, "u1");
  const many = resolveEntityCandidates({ query: "يوسف", candidates: [{ id: "u1", name: "Youssef Ahmed" }, { id: "u2", name: "Yousef Mohamed" }], fields: ["name"], exactFields: ["name"], entityType: "USER" });
  assert.equal(many.entity, null);
  assert.equal(many.guardDecision, "CLARIFY");
});

test("Phase 10 preserves scoped aliases and RBAC-first resolution", async () => {
...
if (ROOT / test_rel).exists():
    ...
    if 'Phase 10 resolves common Arabic-English' not in existing:
        raise SystemExit('PHASE10_V2_RESILIENT_ERROR=IDENTITY_TEST_CONFLICT')
else:
    write(test_rel, test_text)

print('PHASE10_V2_RESILIENT_IDENTITY_FINALIZE=PASS')
