...
def main():
    if len(sys.argv) != 3:
        print("Usage: generate_ux_ui_phase05_project_create_rebased_v2.py <repo> <output.patch>", file=sys.stderr)
        return 2
    ...
    tmp = Path("/var/tmp/tos_ui05_rebased_inner_generator_v2.py")
    ...
    # Unified-diff context lines are prefixed with one literal space. A blank
    # context line is therefore exactly " ", which is valid diff syntax and
    # must not be mistaken for source trailing whitespace. The pinned inner
    # generator already validates every line of the fully rendered source.
    # Here we additionally validate only newly-added source payloads.
    for line_no, line in enumerate(patch_text.splitlines(), start=1):
        if line.startswith("+") and not line.startswith("+++"):
            payload = line[1:]
            if payload.rstrip() != payload:
                raise RuntimeError(f"ADDED_SOURCE_TRAILING_WHITESPACE_PATCH_LINE={line_no}")
    ...
    print("PATCH_WHITESPACE_VALIDATION=ADDED_SOURCE_LINES_ONLY")
    print("VALID_DIFF_BLANK_CONTEXT_LINE_ALLOWED=YES")
