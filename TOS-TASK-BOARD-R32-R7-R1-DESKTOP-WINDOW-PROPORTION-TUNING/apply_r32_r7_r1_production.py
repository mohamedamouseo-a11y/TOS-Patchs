...
PATCH = "TOS-TASK-BOARD-R32-R7-R1-DESKTOP-WINDOW-PROPORTION-TUNING"
...
new_min_width = 'const width = Math.min(440, Math.max(380, Math.round(window.innerWidth * 0.22)));'
if updated.count(old_min_width) != 2:
    fail(f"expected two R32_R7 minimized width expressions, found {updated.count(old_min_width)}")
updated = updated.replace(old_min_width, new_min_width)
...
for contract in (
    ...
    'const minWidth = 620;',
    'const minHeight = 420;',
    'Math.min(440, Math.max(380, Math.round(window.innerWidth * 0.22)))',
    'data-tos-task-window-r32-r7-r1="true"',
):
...
try:
    req = urllib.request.Request("https://tos.tamiyouz.com/tasks", headers={"User-Agent": "TOS-R32-R7-R1-Healthcheck"})
