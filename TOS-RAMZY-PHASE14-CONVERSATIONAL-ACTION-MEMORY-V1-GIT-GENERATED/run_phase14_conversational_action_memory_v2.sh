...
PERSISTENT_GUARD="$PATCH_DIR/02_phase14_persistent_memory_guard.py"
...
echo "RUNNING=RAMZY_PHASE14_CONVERSATIONAL_ACTION_MEMORY_V2"
cd "$TOS_ROOT"
...
for new_file in \
  backend/src/agency-operator/services/ramzyActionDraft.service.js \
  backend/src/agency-operator/tests/ramzyConversationalActionMemoryPhase14.test.js \
  backend/src/agency-operator/tests/ramzyActionDraftPersistentMemoryPhase14.test.js; do
...
[[ -f "$PERSISTENT_GUARD" ]] || fail "PHASE14_PERSISTENT_GUARD_NOT_FOUND"
python3 "$GENERATOR"
echo "PHASE14_MAIN_PATCH=PASS"
python3 "$PERSISTENT_GUARD"
echo "PHASE14_PERSISTENT_MEMORY_GUARD=PASS"
...
# Draft actions must not leak into long-term persistent RamzyMemory.
grep -q 'function looksLikeActionDraftMessage' backend/src/agency-operator/services/ramzyMemory.service.js || fail "PERSISTENT_ACTION_GUARD_MISSING"
...
# Revalidation at draft target write/materialization; execution still rechecks again in Phase 13.
...
grep -q 'executeApprovedTaskAction' backend/src/agency-operator/services/taskCommands.service.js || fail "PHASE13_EXECUTION_RECHECK_PIPELINE_MISSING"

# Conversation continuity and provider-safe view.
...
grep -q 'discard_task_action_draft' backend/src/agency-operator/services/ramzyRuntime.service.js || fail "DRAFT_DISCARD_SIDE_EFFECT_GUARD_MISSING"
