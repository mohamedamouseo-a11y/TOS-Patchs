...
E_CSS = "frontend/src/pages/tws/tSheetsPhase7_2EMicroFinal.css"
E_TEST = "frontend/src/pages/tws/tSheetsPhase7_2EMicroFinal.test.js"

REQUIRED_TWS_DIRTY = {EDITOR, A_CSS, A_TEST, B_CSS, B_TEST, C_CSS, C_TEST, D_CSS, D_TEST}
EXPECTED_BLOBS = {
...
EXPECTED_E_CSS_BLOB = "c77d4863872f51ec9f71bbd600381fecd8e55088"
EXPECTED_E_TEST_BLOB = "24c1180b6e1a76fc8f2ea8f9828c6c425b98a462"

E_CSS_URL = "https://raw.githubusercontent.com/mohamedamouseo-a11y/TOS-Patchs/main/TOS-TWS-GOOGLE-SHEETS-PHASE-7-2E-MICRO-FINAL-FIX/payload/tSheetsPhase7_2EMicroFinal.css"
E_TEST_URL = "https://raw.githubusercontent.com/mohamedamouseo-a11y/TOS-Patchs/main/TOS-TWS-GOOGLE-SHEETS-PHASE-7-2E-MICRO-FINAL-FIX/payload/tSheetsPhase7_2EMicroFinal.test.js"
...
def fetch_bytes(url: str) -> bytes:
    req = urllib.request.Request(url, headers={"User-Agent": "TOS-TWS-Phase72E/1.0", "Cache-Control": "no-cache"})
...
def fetch_text(url: str) -> str:
    req = urllib.request.Request(url, headers={"User-Agent": "TOS-TWS-Phase72E-LiveProbe/1.0", "Cache-Control": "no-cache", "Pragma": "no-cache"})
    with urllib.request.urlopen(req, timeout=45) as response:
...
def fail(message):
    ...
    print("PHASE72A_TO_72D_STATE_PRESERVED=YES")
...
try:
    ...
    if not REQUIRED_TWS_DIRTY.issubset(pre_paths):
        ...
        raise RuntimeError("required Phase 7.2A-D dirty paths missing: " + ", ".join(sorted(missing)))
    ...
    for rel, expected in EXPECTED_BLOBS.items():
        ...
        if actual != expected:
            raise RuntimeError(f"payload blob mismatch for {rel}: expected {expected}, got {actual}")

    for rel in (E_CSS, E_TEST):
        if (REPO / rel).exists():
            raise RuntimeError(f"unexpected pre-existing Phase 7.2E file: {rel}")
    ...
    required_markers = [
        ...
        'fillPreviewRef === ref && "ring-2 ring-inset ring-amber-500"',
        'bg-amber-500 shadow-sm dark:border-zinc-900',
    ]
    missing_markers = [item for item in required_markers if item not in source]
    if missing_markers:
        raise RuntimeError("Phase 7.2D source guard missing: " + ", ".join(missing_markers))
    if 'tSheetsPhase7_2EMicroFinal.css' in source or "tws-sheets-phase7-2e-micro-final" in source:
        raise RuntimeError("Phase 7.2E already appears applied")

    css_payload = fetch_bytes(E_CSS_URL)
    test_payload = fetch_bytes(E_TEST_URL)
    if git_blob_sha(css_payload) != EXPECTED_E_CSS_BLOB:
        raise RuntimeError("Phase 7.2E CSS payload integrity mismatch")
    if git_blob_sha(test_payload) != EXPECTED_E_TEST_BLOB:
        raise RuntimeError("Phase 7.2E test payload integrity mismatch")

    source = replace_once(
        source,
        'import "./tSheetsPhase7_2DDarkFidelity.css";\n',
        'import "./tSheetsPhase7_2DDarkFidelity.css";\nimport "./tSheetsPhase7_2EMicroFinal.css";\n',
        "Phase 7.2E stylesheet import",
    )
    ...
    source = replace_once(
        ...
        "Phase 7.2E root hook",
    )

    source = replace_once(
        source,
        'className="min-w-0 flex-1 truncate border-none bg-transparent text-lg font-black text-zinc-950 outline-none dark:text-white"',
        'className="tws-sheets-title-input min-w-0 flex-1 truncate border-none bg-transparent text-lg font-black text-zinc-950 outline-none dark:text-white"',
        "dark title visibility hook",
    )

    source = replace_once(
        source,
        'fillPreviewRef === ref && "ring-2 ring-inset ring-amber-500"',
        'fillPreviewRef === ref && "ring-2 ring-inset tws-sheets-fill-preview"',
        "fill preview selection color hook",
    )

    source = replace_once(
        source,
        'className="absolute -bottom-1 -right-1 z-30 h-2.5 w-2.5 cursor-crosshair border border-white bg-amber-500 shadow-sm dark:border-zinc-900"',
        'className="tws-sheets-fill-handle absolute -bottom-1 -right-1 z-30 h-2.5 w-2.5 cursor-crosshair border shadow-sm"',
        "fill handle selection color hook",
    )
    ...
    expected_final_paths = pre_paths | {E_CSS, E_TEST}
    ...
    final_required = [
        ...
        "tws-sheets-title-input",
        "tws-sheets-fill-preview",
    ...
    if missing_final:
        raise RuntimeError("Phase 7.2E final source verification missing: " + ", ".join(missing_final))

    if 'fillPreviewRef === ref && "ring-2 ring-inset ring-amber-500"' in final_source:
        raise RuntimeError("amber fill preview residue remains")
    if 'bg-amber-500 shadow-sm dark:border-zinc-900' in final_source:
        raise RuntimeError("amber fill handle residue remains")
    ...
    live_before_html = fetch_text(f"{LIVE_URL}?__phase72e_before={int(time.time())}")
    live_asset_before = extract_main_js(live_before_html) or "UNKNOWN"

    live_index_backup = Path(f"/tmp/tos_phase72e_live_index_{int(time.time())}.html")
    ...
    time.sleep(1)
    live_after_html = fetch_text(f"{LIVE_URL}?__phase72e_after={int(time.time())}")
    live_asset_after = extract_main_js(live_after_html)
    if not live_asset_after:
        raise RuntimeError("could not identify live JS after Phase 7.2E deploy")
    ...
    tmp = Path("/tmp/tos_phase72e_live_asset.js")
    req = urllib.request.Request(
        asset_url(live_asset_after) + f"?v={int(time.time())}",
        headers={"Cache-Control": "no-cache", "User-Agent": "TOS-TWS-Phase72E-LiveProbe/1.0"},
    )
    with urllib.request.urlopen(req, timeout=45) as response:
    ...
    print(f"REPO={REPO}")
    print(f"HEAD={head}")
    print("PRECHECK_WORKTREE=DIRTY_EXPECTED_PHASE72A_TO_PHASE72D")
    if unrelated_dirty:
        print("UNRELATED_DIRTY_PATHS=" + ",".join(sorted(unrelated_dirty)))
    print("PHASE72A_TO_72D_STATE_GUARD=PASS")
    print("PHASE72A_TO_72D_PAYLOAD_BLOBS=PASS")
    print("PHASE72E_PAYLOAD_INTEGRITY=PASS")
    print("PHASE72E_MICRO_FINAL_TESTS=PASS (8/8)")
    ...
    print("DARK_TITLE_VISIBILITY=PASS")
    print("BLUE_SELECTION_UNIFICATION=PASS")
    print("AMBER_FILL_PREVIEW_REMOVED=PASS")
    print("AMBER_FILL_HANDLE_REMOVED=PASS")
    ...
    print("PHASE72E_PATCH_APPLIED=YES")
