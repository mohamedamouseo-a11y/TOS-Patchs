...
PATCH = "TOS-TWS-GOOGLE-SHEETS-PHASE-7-2D-DARK-MODE-FIDELITY-RESET"
SCRIPT = "run_phase7_2d.py"
...
REQUIRED_TWS_DIRTY = {EDITOR, A_CSS, A_TEST, B_CSS, B_TEST, C_CSS, C_TEST}
...
EXPECTED_D_CSS_BLOB = "642190955e523c66224a15539038734b6003da26"
EXPECTED_D_TEST_BLOB = "0424d5979991193f04fb2f83f0a97bbd2b5ccb74"

D_CSS_URL = "https://raw.githubusercontent.com/mohamedamouseo-a11y/TOS-Patchs/main/TOS-TWS-GOOGLE-SHEETS-PHASE-7-2D-DARK-MODE-FIDELITY-RESET/payload/tSheetsPhase7_2DDarkFidelity.css"
D_TEST_URL = "https://raw.githubusercontent.com/mohamedamouseo-a11y/TOS-Patchs/main/TOS-TWS-GOOGLE-SHEETS-PHASE-7-2D-DARK-MODE-FIDELITY-RESET/payload/tSheetsPhase7_2DDarkFidelity.test.js"
...
def fetch_bytes(url: str) -> bytes:
    req = urllib.request.Request(url, headers={"User-Agent": "TOS-TWS-Phase72D/1.0", "Cache-Control": "no-cache"})
...
def fetch_text(url: str) -> str:
    req = urllib.request.Request(url, headers={
        "User-Agent": "TOS-TWS-Phase72D-LiveProbe/1.0",
...
def fail(message):
    ...
    print("PHASE72A_PHASE72B_PHASE72C_STATE_PRESERVED=YES")
...
try:
    ...
    if not REQUIRED_TWS_DIRTY.issubset(pre_paths):
        ...
        raise RuntimeError("required Phase 7.2A/7.2B/7.2C dirty paths missing: " + ", ".join(sorted(missing)))
    ...
    blob_guards = [
        ...
        (C_CSS, EXPECTED_C_CSS_BLOB, "Phase 7.2C CSS"),
        (C_TEST, EXPECTED_C_TEST_BLOB, "Phase 7.2C test"),
    ]
    ...
    for rel in (D_CSS, D_TEST):
        if (REPO / rel).exists():
            raise RuntimeError(f"unexpected pre-existing Phase 7.2D file: {rel}")
    ...
    if missing_markers:
        raise RuntimeError("Phase 7.2A/B/C source guard missing: " + ", ".join(missing_markers))
    if 'tSheetsPhase7_2DDarkFidelity.css' in source or "tws-sheets-phase7-2d-dark-fidelity" in source:
        raise RuntimeError("Phase 7.2D already appears applied")

    css_payload = fetch_bytes(D_CSS_URL)
    test_payload = fetch_bytes(D_TEST_URL)
    if git_blob_sha(css_payload) != EXPECTED_D_CSS_BLOB:
        raise RuntimeError("Phase 7.2D CSS payload integrity mismatch")
    if git_blob_sha(test_payload) != EXPECTED_D_TEST_BLOB:
        raise RuntimeError("Phase 7.2D test payload integrity mismatch")

    source = replace_once(
        source,
        'import "./tSheetsPhase7_2CFinal.css";\n',
        'import "./tSheetsPhase7_2CFinal.css";\nimport "./tSheetsPhase7_2DDarkFidelity.css";\n',
        "Phase 7.2D stylesheet import",
    )
    ...
    source = replace_once(
        ...
        'tws-sheets-phase7-premium tws-sheets-phase7-1-restructure tws-sheets-phase7-2-google-parity tws-sheets-phase7-2b-polish tws-sheets-phase7-2c-final tws-sheets-phase7-2d-dark-fidelity relative flex h-full flex-col',
        "Phase 7.2D root hook",
    )

    source_applied = True
    editor_path.write_text(source, encoding="utf-8")
    (REPO / D_CSS).write_bytes(css_payload)
    (REPO / D_TEST).write_bytes(test_payload)
    ...
    expected_final_paths = pre_paths | {D_CSS, D_TEST}
    ...
    if missing_final:
        raise RuntimeError("Phase 7.2D final source verification missing: " + ", ".join(missing_final))
    ...
    live_before_html = fetch_text(f"{LIVE_URL}?__phase72d_before={int(time.time())}")
    live_asset_before = extract_main_js(live_before_html) or "UNKNOWN"

    live_index_backup = Path(f"/tmp/tos_phase72d_live_index_{int(time.time())}.html")
    ...
    time.sleep(1)
    live_after_html = fetch_text(f"{LIVE_URL}?__phase72d_after={int(time.time())}")
    live_asset_after = extract_main_js(live_after_html)
    if not live_asset_after:
        raise RuntimeError("could not identify live JS after Phase 7.2D deploy")
    ...
    tmp = Path("/tmp/tos_phase72d_live_asset.js")
    req = urllib.request.Request(
        asset_url(live_asset_after) + f"?v={int(time.time())}",
        headers={"Cache-Control": "no-cache", "User-Agent": "TOS-TWS-Phase72D-LiveProbe/1.0"},
    )
    with urllib.request.urlopen(req, timeout=45) as response:
    ...
    print(f"REPO={REPO}")
    print(f"HEAD={head}")
    print("PRECHECK_WORKTREE=DIRTY_EXPECTED_PHASE72A_PHASE72B_PHASE72C")
    if unrelated_dirty:
        print("UNRELATED_DIRTY_PATHS=" + ",".join(sorted(unrelated_dirty)))
    print("PHASE72A_PHASE72B_PHASE72C_STATE_GUARD=PASS")
    print("PHASE72A_PHASE72B_PHASE72C_PAYLOAD_BLOBS=PASS")
    print("PHASE72D_PAYLOAD_INTEGRITY=PASS")
    print("PHASE72D_DARK_FIDELITY_TESTS=PASS (10/10)")
    ...
    print("DARK_CHARCOAL_SURFACE_RESET=PASS")
    print("DARK_TOOLBAR_WHITE_ISLAND_REMOVAL=PASS")
    print("DARK_FORMULA_BAR_RESET=PASS")
    print("DARK_TITLE_FIELD_RESET=PASS")
    print("BLUE_SPREADSHEET_SELECTION_LANGUAGE=PASS")
    print("DARK_AXIS_HIGHLIGHT_RESET=PASS")
    print("DARK_SHEET_TAB_ADD_RESET=PASS")
    ...
    print("PHASE72D_PATCH_APPLIED=YES")
