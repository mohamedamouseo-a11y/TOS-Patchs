...
THREAD_END = "function MessageActions("
...
REPLACEMENTS = [
    (
        ...
        '    <div className="fixed inset-0 z-40 bg-zinc-950/25 backdrop-blur-sm lg:inset-auto lg:bottom-0 lg:right-0 lg:top-0 lg:w-[400px]" role="dialog" aria-modal="true">',
        1,
    ),
    (
        ...
        '      <aside className="relative mr-auto flex h-full w-full max-w-[400px] flex-col border-r border-zinc-100 bg-white shadow-xl dark:border-white/10 dark:bg-zinc-950 lg:mr-0">',
        1,
    ),
    (
    ...
    (
        ...
        '          <div className="rounded-[18px] border border-amber-100 bg-amber-50 p-3 text-[13px] text-amber-900 dark:border-amber-500/20 dark:bg-amber-500/10 dark:text-amber-100">',
        1,
    ),
    (
    ...
    (
        ...
        '              <div key={reply.id} className="rounded-xl bg-zinc-50 p-2.5 text-[13px] dark:bg-white/5">',
        1,
    ),
    (
...
def main():
    ...
    if before.count(THREAD_END) != 1:
        raise RuntimeError(f'THREAD_END_COUNT={before.count(THREAD_END)}; expected 1')

    start = before.index(THREAD_START)
    end = before.index(THREAD_END, start)
    prefix = before[:start]
    ...
    if after[:len(prefix)] != prefix:
        raise RuntimeError('OUTSIDE_THREAD_PREFIX_CHANGED')
    if after[len(prefix) + len(updated):] != suffix:
        raise RuntimeError('OUTSIDE_THREAD_SUFFIX_CHANGED')
