...
TOS_BEFORE="$(git -C "$TOS" status --porcelain=v1)"
...
TOS_AFTER="$(git -C "$TOS" status --porcelain=v1)"
[ "$TOS_BEFORE" = "$TOS_AFTER" ] || fail "TOS_WORKTREE_CHANGED"
