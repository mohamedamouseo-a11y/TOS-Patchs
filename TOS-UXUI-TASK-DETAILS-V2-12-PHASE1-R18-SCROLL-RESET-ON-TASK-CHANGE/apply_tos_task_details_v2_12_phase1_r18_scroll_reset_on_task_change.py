...
for path in (FRONTEND, STYLE_DIR, BOARD, APP, SIDEBAR, R17_STYLE, MANIFEST, PAYLOAD):
    if not path.exists():
...
r17_source = R17_STYLE.read_text()
payload_css = PAYLOAD.read_text()

if R17_MARKER not in r17_source:
    fail("required R17 baseline marker missing")
...
if r17_import not in updated_board:
    fail("R17 stylesheet import missing")
...
try:
    ...
    for marker in (R18_MARKER, R17_MARKER):
...
print("R17_ASSIGNEE_PORTAL_PRESERVED=YES")
