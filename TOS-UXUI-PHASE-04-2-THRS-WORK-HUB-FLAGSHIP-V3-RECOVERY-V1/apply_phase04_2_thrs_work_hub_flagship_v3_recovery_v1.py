...
print("RUNNING=PHASE04_2_THRS_WORK_HUB_FLAGSHIP_V3_RECOVERY_V1")
...
def fail(message: str):
    ...
    print("V3_RUNTIME=NO")
    print("V3_RECOVERY_V1=NO")
    sys.exit(1)


def verify_added_lines_have_no_trailing_whitespace():
    ...
    for line in diff.splitlines():
        if not line.startswith("+") or line.startswith("+++"):
            continue
        added = line[1:]
        if added.endswith(" ") or added.endswith("\t"):
            bad.append(line)
    if bad:
        raise RuntimeError("targeted whitespace validation failed on added lines")
...
# Semantic visual hooks only. No state/API/permission/workflow changes.
replacements = [
    (
...
try:
    ...
    # The original V3 was blocked by git diff --check despite the source being
    # rolled back cleanly. Validate only the actual added lines here so no
    # unrelated/pre-existing whitespace can block this visual patch.
    verify_added_lines_have_no_trailing_whitespace()
    ...
    stage = LIVE_PARENT / f"build.phase04-2-workhub-v3-recovery-v1.new.{int(time.time())}"
    backup = LIVE_PARENT / f"build.phase04-2-workhub-v3-recovery-v1.backup-{stamp}"
    if stage.exists():
        shutil.rmtree(stage)
...
print("V3_RECOVERY_V1=YES")
print("TARGETED_WHITESPACE_VALIDATION=PASS")
