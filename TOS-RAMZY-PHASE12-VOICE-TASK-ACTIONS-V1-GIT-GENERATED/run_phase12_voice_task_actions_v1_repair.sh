...
REPAIR="$PATCH_DIR/02_phase12_task_command_anchor_repair.py"
...
fail() {
  echo "PHASE12_VOICE_TASK_ACTIONS_REPAIR=FAIL"
  echo "REASON=$1"
  exit 1
}
...
is_target() { in_array "$1" "${TARGET_FILES[@]}"; }
...
public_code() {
  local method="$1" path="$2" body="${3:-}" code="000"
...
echo "RUNNING=RAMZY_PHASE12_VOICE_TASK_ACTIONS_V1_REPAIR"
cd "$TOS_ROOT"
...
while IFS= read -r line; do
  ...
  if is_target "$path"; then
    in_array "$path" "${KNOWN_PARTIAL_FILES[@]}" || fail "UNEXPECTED_DIRTY_PHASE12_TARGET:$path"
    PARTIAL_COUNT=$((PARTIAL_COUNT + 1))
    case "$path" in
      backend/src/agency-operator/services/semanticIntentResolver.service.js)
        grep -q '"CREATE_TASK"' "$path" || fail "KNOWN_PARTIAL_SEMANTIC_MARKER_MISSING" ;;
      backend/src/agency-operator/policies/agentAccess.service.js)
        grep -q 'assertAgentTaskCreateAccess' "$path" || fail "KNOWN_PARTIAL_ACCESS_MARKER_MISSING" ;;
      backend/src/agency-operator/services/actionGroundingValidation.service.js)
        grep -q 'TASK_CREATE_GROUNDED_AND_AUTHORIZED' "$path" || fail "KNOWN_PARTIAL_GROUNDING_MARKER_MISSING" ;;
      backend/src/agency-operator/services/ramzySystemIntelligence.service.js)
        grep -q 'detectedIntent === "TASK_CREATE"' "$path" || fail "KNOWN_PARTIAL_INTELLIGENCE_MARKER_MISSING" ;;
    esac
  fi
done <<< "$PRE_STATUS"

if [[ "$PARTIAL_COUNT" -gt 0 ]]; then
  echo "PRE_STATE=KNOWN_PHASE12_PARTIAL"
  echo "KNOWN_PARTIAL_TARGET_COUNT=$PARTIAL_COUNT"
else
  echo "PRE_STATE=TARGET_FILES_CLEAN"
fi

[[ -f "$REPAIR" ]] || fail "REPAIR_HELPER_NOT_FOUND"
python3 "$REPAIR"
echo "PATCH_APPLY=PASS"

# Contracts
for check in \
  'backend/src/agency-operator/services/semanticIntentResolver.service.js|"CREATE_TASK"' \
  'backend/src/agency-operator/policies/agentAccess.service.js|assertAgentTaskCreateAccess' \
  'backend/src/agency-operator/services/actionGroundingValidation.service.js|TASK_CREATE_GROUNDED_AND_AUTHORIZED' \
  'backend/src/agency-operator/services/ramzySystemIntelligence.service.js|detectedIntent === "TASK_CREATE"' \
  'backend/src/agency-operator/services/taskCommands.service.js|actionType === "CREATE_TASK"' \
  'backend/src/agency-operator/services/taskCommands.service.js|targetType = "PROJECT"' \
  'backend/src/agency-operator/services/taskCommands.service.js|ensureProjectDefaultWorkspaceBoard' \
  'backend/src/agency-operator/tools/createRamzyTools.js|"CREATE_TASK"' \
  'backend/src/routes/agent.routes.js|isCreateTaskApproval' \
  'backend/src/routes/agent.routes.js|assertAgentTaskCreateAccess' \
  'backend/src/agency-operator/prompts/ramzyPrompt.js|Phase 12:' \
  'frontend/src/components/RamzyAssistant.jsx|CREATE_TASK: { ar: "إنشاء مهمة", en: "Create task" }'; do
  file="${check%%|*}"; pattern="${check#*|}"
  grep -Fq "$pattern" "$file" || fail "CONTRACT_MISSING:$file:$pattern"
done
...
for file in "${TARGET_FILES[@]}"; do
  printf '%s\n' "$POST_STATUS" | grep -Fq " $file" || fail "EXPECTED_PHASE12_FILE_NOT_CHANGED:$file"
done
...
BACKEND_PM2=""
if pm2 describe tamiyouz-system >/dev/null 2>&1; then BACKEND_PM2="tamiyouz-system";
elif pm2 describe tamiyouz-backend >/dev/null 2>&1; then BACKEND_PM2="tamiyouz-backend";
else fail "BACKEND_PM2_NOT_FOUND"; fi
...
diff -qr /var/www/TOS/frontend/dist /opt/apps/tamiyouz-front/build >/dev/null || fail "DEPLOYED_DIST_MISMATCH"
echo "DEPLOYED_DIST_MATCH=PASS"
echo "NO_COMMIT_OR_PUSH=YES"
echo "PHASE12_VOICE_TASK_ACTIONS_REPAIR=PASS"
